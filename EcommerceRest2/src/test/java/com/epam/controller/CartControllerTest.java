...
class CartControllerTest {
	...
	void testAddToCart() {
		...
		cartItem.setCartId((long) 1);
		cartItem.setProduct(apple);
		cartItem.setQuantity((long) 1);
		cartItem.setShoppingCart(shoppingCart);
		when(cartService.addToCart(apple.getProductId(), (long) 1)).thenReturn(cartItem);
		Assertions.assertNotNull(cartController.addToCart(apple.getProductId(), (long) 1));
	}
	...
	void testUpdateCart() {
		...
		cartItem.setQuantity((long) 1);
		cartItem.setShoppingCart(shoppingCart);
		when(cartService.updateCart(apple.getProductId(), (long) 1)).thenReturn(cartItem);
		Assertions.assertNotNull(cartController.updateCart(apple.getProductId(), (long) 1));
	}
