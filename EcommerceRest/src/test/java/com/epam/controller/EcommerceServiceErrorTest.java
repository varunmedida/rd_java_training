...
class EcommerceServiceErrorTest {
	...
	void testHandleDataNotFoundException() {
		Assertions.assertNotNull(serviceError.handleDataNotFoundException(exception, headers, HttpStatus.NOT_FOUND, request));
	}

	@Test
	void testHandleInsufficientQuantityException() {
		Assertions.assertNotNull(serviceError.handleInsufficientQuantityException(exception2, headers, HttpStatus.BAD_REQUEST, request));
	}
